...
f1B <- ggplot(plot.data, aes(x = tSNE_1, y = tSNE_2, colour = Cluster)) +
    geom_point(data = filter(plot.data, Cluster %in% c(2, 9)),
...
genes <- c("TCF21", "PODXL", "VEGFA", "NPHS1", "PTPRO", "CTGF", "OLFM3", "MAFB",
           ...
           "HNF1B", "EPCAM", "EMX2", "SPP1", "MAL", "PAX2")

gene.groups <- c(rep("Podocyte", 5),
                 rep("Pod. precursor", 3),
...
plot.data <- FetchData(orgs.neph, vars.all = genes) %>%
    data.frame() %>%
    rownames_to_column("Cell") %>%
    mutate(Cluster = paste0("ON", orgs.neph@ident)) %>%
    gather(key = "Gene", value = "Expr", -Cell, -Cluster) %>%
    group_by(Cluster, Gene) %>%
    summarize(AvgExpr = ExpMean(Expr),
              Prop = Seurat:::PercentAbove(Expr, threshold = 0)) %>%
    group_by(Gene) %>%
    mutate(AvgExp = scale(AvgExpr)) %>%
    mutate(Group = gene.groups[Gene])

f1E <- ggplot(plot.data,
              aes(x = Gene, y = Cluster, size = Prop, colour = AvgExpr)) +
    geom_point() +
    scale_colour_viridis_c() +
    scale_radius(range = c(0, 10)) +
    facet_grid(~ Group, scale = "free_x", space = "free_x", switch = "y") +
...
```{r fig1-panel, fig.height = 20, fig.width = 16}
p1 <- plot_grid(f1A, f1B, f1C, f1D, nrow = 2,
                labels = c("A", "B", "C", "D"))
panel <- plot_grid(p1, f1E, ncol = 1, labels = c("", "E"),
                   rel_heights = c(1, 0.2))
...
kable(data.frame(
    File = c(
        ...
        glue("[figure1E.png]({getDownloadURL('figure1E.png', DOCNAME)})"),
        glue("[figure1E.pdf]({getDownloadURL('figure1E.pdf', DOCNAME)})"),
