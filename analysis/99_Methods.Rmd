...
```{r pkg-bib}
write_bib(c("SingleCellExperiment"), file = here("data/packages.bib"))
```
...
barcode processing and single-cell gene counting [ref]. Briefly, samples were
...
(v`r str_extract(R.version.string, "[0-9\\.]+")`) [ref] was used for further 
...
[@Zappia2018-ph] showing the expression of known marker genes to select the 
...
The combined organoid and human foetal kidney analysis used the procedure
...
Figures shown here were produced using...

Availability
============

The datasets are available from GEO accession numbers XXX. A website showing
reports produced during analysis including the exact software versions and
parameters used can be accessed at XXX and the code is available at XXX.
